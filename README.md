...
🧩 전투 관련 기능
1. 전투 개시 커스텀 명령어 (StartBattleCommand) 확장
전투 시작 시 적 데이터, 배경, 배경음악 설정

첫 번째 턴을 플레이어 또는 적으로 지정 가능 (firstTurn)

보상 아이템 목록 전달 가능 (rewardList)

2. 전투 보상 시스템
전투 종료 후 "YOU DIED" 또는 "YOU WIN" 메시지 표시

보상이 있을 경우 아이템 보상 패널 표시

플레이어가 보상 아이템 중 1개 선택 가능

보상이 없으면 메세지만 띄우고 넘어감

3. 스킬 사용 시 슬로우 연출 및 카메라 전환
스킬 사용 시 카메라 전환 → 연출 → 다시 복귀

슬로우 시간 동안 상대 턴도 동시 실행 가능하게 개선 예정

⚔️ 무기/스킬/숙련도 시스템
4. 무기 종류 시스템 도입
무기마다 WeaponType(검, 창 등) 할당

같은 무기 종류 간 숙련도 공유

5. 무기 숙련도 시스템
Player 클래스에 weaponProficiency 딕셔너리 도입

무기 사용 시 숙련도 증가 (AddProficiency)

숙련도에 따라 스킬 해금

6. 스킬 해금 조건 추가
Skill에 requiredProficiencyLevel 필드 추가

해당 레벨 이상일 때만 스킬 해금됨

7. 스킬 해금 시 보상으로 표시
전투 중 숙련도 증가로 해금된 스킬을 추적

전투 종료 후 기존 보상과 함께 보상 패널에 표시

📋 UI 및 전반적인 개선사항
8. 스킬슬롯 필터링
인벤토리나 스킬창에서 현재 무기의 숙련도에 따라 보이는 스킬 제한

9. 코드 구조 및 유지보수성 개선
BattleManager, Player, CustomLineView 등 핵심 클래스의 책임 분리

단일 책임 원칙(SRP)에 맞게 각 기능의 책임 범위 명확히 정리됨
