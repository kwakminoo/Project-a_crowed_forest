...
# [전투 시스템 개선] 플레이어 턴 연출 및 UI 재배치

## 🎯 목표
전투 중 플레이어의 턴을 시각적으로 강조하고, 전투 UI(스킬 슬롯 및 아이템 슬롯)를 캐릭터 중심으로 시각화하여 전투의 몰입도를 높인다.
...
### ✅ 1. 스킬/아이템 슬롯 UI 배치
- `Canvas` 내에 **RadialMenu** 형태의 UI 생성
- 플레이어 캐릭터 기준으로 원형으로 배치된 스킬/아이템 슬롯 구성
- 슬롯은 기본 상태에선 숨겨진 상태이며, 플레이어의 턴일 때만 표시됨

### ✅ 2. UI 활성화 타이밍
- **플레이어 턴 시작 시** 슬롯 UI 자동 활성화
- **슬로우모션 중**에도 표시 가능
- 스킬 또는 아이템 사용 후 자동으로 숨김 처리

### ✅ 3. 카메라 연출 개선
- 플레이어 턴일 때 카메라가 캐릭터에 **줌인/고정**
- 슬로우모션 타이밍에 UI 강조
- 행동 완료 시 카메라는 원래 상태로 복귀

### ✅ 4. 슬롯 애니메이션
- RadialMenu는 `Scale` 애니메이션으로 자연스럽게 확장/축소
- `ShowMenu()` / `HideMenu()` 메서드 제공

### ✅ 5. 전투 UI와 연계
- 스킬 슬롯 버튼 → `BattleManager.UseSkill(int index)`
- 아이템 슬롯 버튼 → `Inventory.UseItem(...)`
...
## 🧩 참고 사항
- UI는 기본적으로 비활성화 (`SetActive(false)`)
- 전투 턴의 흐름에 따라 상태 자동 전환
- `Time.timeScale` 슬로우 적용 시, **연출 타이밍 고려 필요**
...
- 도입 스탯:
  - **체력(HP)**: 생존력
  - **정신력(Sanity)**: 정신 상태를 반영, 붕괴 트리거
  - **근력(Strength)**: 무기 공격력/스킬 공격력 증가
  - **기량(Dexterity)**: 회피율/속도 관련
  - **운(Luck)**: 드랍율, 치명타율, 붕괴 확률 영향
- **확장 가능**하도록 설계
...
- 무기마다 **기본 공격력** 설정
- 무기 장착 시:  
  `기본 공격력 = 플레이어 근력 + 무기 공격력 + 숙련도 보너스`
- 무기 숙련도 상승 시 무기 공격력도 소폭 증가

---

### 🧠 3. 정신력 붕괴 시스템
- 정신력 하락 시 붕괴 상태 진입
- 확률적 붕괴 진입 시 아래 상태 중 하나 부여

| 이름 | 효과 |
|------|------|
| **이해할 수 없는 공포** | 회피율 상승, 공격력 저하 |
| **그날의 분노** | 공격력 상승, 스킬 사용 시 HP 감소 |
| **좀먹는 광기** | 확률적 추가 공격 & 자해 발생 |
| **이상향** | 스킬 사용을 확률적으로 강제/거부 |
| **날 죽이지 못하는 고통은 날 더 강하게 만들 뿐이다** | *버프*: 공격력↑, 체력흡수, 정신력 소폭 회복, 회피율↑, 추가 공격 확률 |

- 붕괴는 전투 중 **특수 상태이상처럼 처리**

---

## 🛠️ UI 확장 및 사용 방식
- 스킬 슬롯 외에 **아이템 슬롯 UI 추가**
- 전투 시작 시 플레이어 위/아래 **곡선형 배치**로 슬롯 표시
- 플레이어 턴 또는 슬로우 타임 시 **카메라 확대 + 슬롯 노출**
- 행동 선택 시 다시 슬롯 숨김

---

## 📦 보상 시스템 통합
- Yarn Command에서 보상 아이템/스킬 입력 가능
- 무기 숙련도 상승에 따른 스킬 해금도 **보상 패널에 표시**
- 여러 출처의 보상은 **중복 없이 병합**
...
| **냉철한 자** | 정신력 손실 감소 |
| **잔혹한 손길** | 출혈 속성 공격 강화 |
| **기계적인 반사** | 일정 확률로 스킬 회피 |
| **불굴의 신념** | 정신붕괴 확률 감소 |
| **연금술사** | 아이템 효과 +25% 증가 |

### 🔧 구현 방향
- `Player` 클래스에 `List<Trait> activeTraits` 추가
- 아이템, 무기, 스토리 이벤트에서 트리거되는 방식 설계
- 중복 특성 방지 & 자동 해제 시스템 포함
...
### 📌 목표
- **텍스트 어드벤처 + 시각적 맵 UI** 결합
- 플레이어가 탐색 가능하며, 각 노드는 Yarn 대화나 전투, 이벤트와 연결
...
1. **맵 노드 (MapNode)**
   - 위치 좌표
   - 연결된 노드 정보
   - 해당 노드에서 발생할 수 있는 이벤트
     - 전투 / 상호작용 / 이야기

2. **맵 UI**
   - 미니맵 또는 전체 맵 방식
   - 플레이어의 현재 위치를 실시간 표시
   - 노드 클릭 시 해당 지역으로 이동 → Yarn 노드 실행

3. **노드 조건**
   - 열쇠 필요 / 특정 스토리 조건 필요
   - 일정 턴 후에만 활성화 등 조건식 가능
...
| 카테고리 | 기능 |
|----------|------|
| 전투 | 턴제 + 슬로우 반응, 속성 부여, 정신붕괴 |
| 플레이어 성장 | 무기 숙련도, 스킬 해금, 무기 공격력 |
| UI/UX | 전투 종료 UI, 전투 HUD 개선, 아이템 슬롯 |
| Yarn 연동 | 전투 설정, 보상 지정 |
| 특성 시스템 | 보유/해금/상호작용형 트레잇 |
| 탐색 맵 | 노드 기반 탐색, 조건 이동, 이벤트 트리거 |
...
몬스터, npc등이 들어있는 도감 완성하기

고문실 타이틀에서 텍스트가 다 뜨기도 전에 이미 선택지가 떠있음 일단 이부분만 이럼
...
카인은 혼의 까마귀숲의 용병으로 시작할 것 같음 플레이어 선택에 따라 의뢰를 받으면 그 루트를 타게 됨
크게 도시 안과 밖으로 나뉘며 도시속에서 마피아, 귀족, 혁명가, 부유한 상인, 등 여러 엔딩을 가질 수 있고 도시 밖에선 광신도, 용병, 탐험가, 신도 등 엔딩을 생각중
