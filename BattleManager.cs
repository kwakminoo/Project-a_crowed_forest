...
public class BattleManager : MonoBehaviour
{
    ...
    /// <summary>
    /// 리팩토링된 컴포넌트 초기화 (현재 비활성화)
    /// </summary>
    /*
    private void InitializeRefactoredComponents()
    {
        // BattleStateManager 초기화
        if (stateManager == null)
        {
            stateManager = GetComponent<BattleStateManager>();
            if (stateManager == null)
            {
                stateManager = gameObject.AddComponent<BattleStateManager>();
                Debug.Log("BattleStateManager가 자동으로 추가되었습니다.");
            }
        }
        ...
        if (stateManager != null)
        {
            stateManager.OnStateChanged += HandleBattleStateChanged;
        }
    }
    */
